...
export default function AuthorizationVerificationPage() {
  ...
  useEffect(() => { if (!getConfig().configured) return; async function load() { try { const client = createReadClient(); const next = await readAuthorization(client, id); setAuthorization(next); const nextPurchase = await readPurchase(client, next.purchase_id); setPurchase(nextPurchase); setAssessment(await readAssessment(client, next.assessment_id)); } catch (cause) { setError(cause instanceof Error ? cause.message : "Authorization could not be read"); } } void load(); }, [id]);
  ...
  if (error) return <><PageHeader eyebrow="Verification" title={id} /><EmptyState title="No authorization exists for this ID." description="The verifier does not infer why a record is missing." action={<Button href="/verify">Try another ID</Button>} /></>;
